...
class ImprovedVideoMLPTrainer:
    ...
    def train_improved_mlp(self, cv_folds=3):
        ...
        # Entrenar modelo final (sin class_weight)
    ...
    def plot_mlp_training_results(self):
        """
        Gráficas similares al LSTM: pérdida y distribución de predicciones.
        """
        if not hasattr(self.model, 'loss_curve_'):
            print("⚠️ No hay historial de pérdida disponible.")
            return

        fig, axes = plt.subplots(2, 2, figsize=(14, 10))

        # 1. Curva de pérdida
        axes[0, 0].plot(self.model.loss_curve_)
        ...
        axes[1, 0].hist(max_probs, bins=10, color='purple', edgecolor='black', alpha=0.7)
        axes[1, 0].set_title("Confianza del MLP en las predicciones")
        axes[1, 0].set_xlabel("Probabilidad Máxima")
        axes[1, 0].set_ylabel("Frecuencia")
        axes[1, 0].grid(True)

        # 4. Comparación Real vs Predicho (gráfico de conteo)
        import pandas as pd
        df_pred = pd.DataFrame({
            'Real': [class_names[i] for i in self.y_test],
            'Predicho': [class_names[i] for i in y_pred]
        })
        sns.countplot(data=df_pred, x='Real', hue='Predicho', palette='Set2', ax=axes[1, 1])
        axes[1, 1].set_title("Distribución Real vs Predicho")
        axes[1, 1].set_xlabel("Clase Real")
        axes[1, 1].set_ylabel("Cantidad")
        axes[1, 1].legend(title="Predicho")
    ...
    def evaluate_final(self):
        ...
        class_names = self.label_encoder.classes_  # ⬅️ Mover esta línea AQUÍ
        ...
        # Solo calcular ROC AUC si es problema binario
...
def plot_test_results(y_test, y_pred, y_proba, label_encoder):
    import seaborn as sns
    import matplotlib.pyplot as plt
    import pandas as pd
    from sklearn.metrics import confusion_matrix
    ...
    df = pd.DataFrame({
        ...
        'Predicho': [class_names[i] for i in y_pred],
        'Confianza': [round(p[pred], 2) for p, pred in zip(y_proba, y_pred)]
    })
    ...
    plt.title("Distribución Real vs Predicho en Test")
    ...
    plt.figure(figsize=(6, 5))
    sns.histplot(df['Confianza'], bins=10, kde=True, color='green')
    plt.title("Confianza del MLP en Predicciones (Test)")
    plt.xlabel("Confianza en la Clase Predicha")
...
def main():
    ...
    try:
        ...
        n_features = int(input("🎯 Número de características a seleccionar [30]: ") or "30")
        trainer.prepare_data_with_feature_selection(test_size=0.2, n_features=n_features)
        
        # Entrenar
        model = trainer.train_improved_mlp(cv_folds=5)
        ...
        if ejecutar_grid == 's':
            param_grid = [
                {'hidden_layer_sizes': (8,), 'activation': 'relu', 'alpha': 0.01},
                {'hidden_layer_sizes': (16,), 'activation': 'tanh', 'alpha': 0.01},
                {'hidden_layer_sizes': (32,), 'activation': 'relu', 'alpha': 0.001},
                {'hidden_layer_sizes': (32, 16), 'activation': 'relu', 'alpha': 0.0005},
                {'hidden_layer_sizes': (64, 32), 'activation': 'tanh', 'alpha': 0.0001},
                {'hidden_layer_sizes': (16, 8), 'activation': 'relu', 'alpha': 0.0005},
                {'hidden_layer_sizes': (24, 12), 'activation': 'tanh', 'alpha': 0.005}
            ]
            ...
            for i, config in enumerate(param_grid):
                ...
                puntajes = cross_val_score(modelo, trainer.X_train, trainer.y_train, cv=cv, scoring='f1_weighted')
                resultados.append({
                    'config': config,
                    'mean_f1': puntajes.mean(),
                    'std_f1': puntajes.std()
                })
                print(f"   F1-Score: {puntajes.mean():.4f} ± {puntajes.std():.4f}")

            df_resultados = pd.DataFrame(resultados)
            df_resultados = df_resultados.sort_values(by='mean_f1', ascending=False)
            df_resultados.to_csv("resultados_grid_mlp.csv", index=False)
            print("\n✅ Resultados guardados en 'resultados_grid_mlp.csv'")
        ...
        trainer.plot_mlp_training_results()
        plot_test_results(trainer.y_test, trainer.model.predict(trainer.X_test), trainer.model.predict_proba(trainer.X_test), trainer.label_encoder)

        # Mostrar características importantes
        importance_df = pd.DataFrame({
            'feature': trainer.selected_features,
            'importance': np.mean(np.abs(trainer.model.coefs_[0]), axis=1)
